...
public class BigDecimal extends Number implements Serializable, Comparable {
   ...
   public BigDecimal(char[] inchars, int offset, int length) {
      ...
      for(i = offset; i <= $1; ++i) {
         ...
         if(si == 48) {
            ...
            if(si <= 57 || UCharacter.digit(si, 10) != 0) {
               break;
            }
   ...
   public String format(int before, int after, int explaces, int exdigits, int exformint, int exround) {
      ...
      if(explaces > 0) {
         ...
         for(p = a.length - 1; $13 > 0 && a[p] != 69; --p) {
            --$13;
         }
      ...
      if(this.form != 0) {
         ...
         if(this.form == 1) {
            ...
            if(sig >= cmant.length) {
               ...
               for($18 = sig - cmant.length; $18 > 0; --$18) {
                  sb.append('0');
               }
            } else {
         ...
         if(mag < 1) {
            ...
            for(i = needsign + cmant.length; $18 > 0; ++i) {
               rec[i] = 48;
               --$18;
            }
   ...
   private BigDecimal dodivide(char code, BigDecimal rhs, MathContext set, int scale) {
      ...
      if(rhs.ind == 0) {
         ...
         return set.form != 0?ZERO:(scale == -1?this:this.setScale(scale));
      } else {
         int reqdig = set.digits;
         if(reqdig > 0) {
            if(this.mant.length > reqdig) {
         ...
         if(newexp < 0 && code != 68) {
            return code == 73?ZERO:clone(lhs).finish(set, false);
         } else {
            BigDecimal res = new BigDecimal();
            res.ind = (byte)(lhs.ind * rhs.ind);
            res.exp = newexp;
            ...
            while(true) {
               ...
               if(scale >= 0 && -res.exp > scale || code != 68 && res.exp <= 0) {
                  break;
               }
