...
public class PropsVectors {
   ...
   public void setValue(int start, int end, int column, int value, int mask) {
      if(start >= 0 && start <= end && end <= 1114113 && column >= 0 && column < this.columns - 2) {
         if(this.isCompacted) {
            ...
            firstRow = firstRow + column;
            lastRow = lastRow + column;
            mask = ~mask;
   ...
   public void compact(PropsVectors.CompactHandler compactor) {
      if(!this.isCompacted) {
         ...
         for(int i = 0; i < this.rows; ++i) {
            int start = this.v[indexArray[i].intValue()];
            if(count < 0 || !this.areElementsSame(indexArray[i].intValue() + 2, this.v, indexArray[i - 1].intValue() + 2, valueColumns)) {
               count += valueColumns;
            }
         ...
         for(int i = 0; i < this.rows; ++i) {
            ...
            if(count < 0 || !this.areElementsSame(indexArray[i].intValue() + 2, temp, count, valueColumns)) {
               count += valueColumns;
